...
\begin{frame}[shrink]{Higher Observables Constraint Algebra}
	\begin{defpropblock}[$L_\infty$-observables constraint algebra]
		...
		where: 
			\begin{align*}
				\small\Omega_0^k &:=~\{\alpha \in \Omega^k(M)~|~\alpha(v_1,...,v_k)\in  I_S~\forall v_i\in \X_N\} 
				\\
				\mathfrak{X}_N &:=~ 
				\left\lbrace
					v \in \X(M)
	...
	\vfill
	\pause
	Hence, the subquotient $\frac{\Ham_N}{\Ham_0}$ of $\Ham_T$ forms an $L_\infty$-algebra.
\end{frame}
\note[itemize]{
...
\end{frame}
\note[itemize]{
	\item We will develop a multisymplectic reduction in this talk, symplectically is (see slide).
	\item Such a formula can be more generally applied to a Poisson algebra $P$ with associative ideals $I\subset J$ such that $\{I,I\}\subset I$ and $\{I,J\}\subset J$:
$$
