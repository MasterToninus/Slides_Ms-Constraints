...
\note[itemize]{
		...
		\item 	{Def. Symplectic manifold $(M,\omega)$} $M$ a smooth manifold, $\omega\in \Omega^{2}(M)$ closed and non-degenerate, i.e.:
		\begin{itemize} 	
			\item $d\omega=0$
	...
	\item   Examples:
		\begin{itemize}
			\item orientable 2-manifolds with volume, e.g. $S^2$, $T^2$, $\mathbb R^2$
			\item cotangent bunlde $T^*Q$ of any manifold $Q$, with $\omega=d\theta$. $\theta_\eta(v)=\eta(T\pi_Q(v))$.
...
\begin{frame}[fragile]{Attempting algebraic reduction}
...
\begin{frame}{Constraint Algebras} 
	\begin{block}{Idea:}
	    \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
			...
			\item[-] Let's work in a category where all data needed for the reduction are bundled together in suitable categorical objects. {\tiny (To avoid pathologies arising from these steps)}
	    \end{itemize}
	\end{block}
	\pause
	\vfill
...
manifold to any classical field system (e.g. a continuous medium like a filament or a fluid). See frame Extra-\ref{Frame:Ms-Field-Mechanics} 
	...
	\item Examples:
		\begin{itemize} 
			\item orientable $n{+}1$-manifolds with volume, e.g. $S^{n+1}$, $T^{n+1}$, $\mathbb R^{n+1}$ 
			\item multicotangent bunlde $\Lambda^nT^*Q$ of any manifold $Q$, with $\omega=d\theta$. $\theta_\eta(v_1,...,v_n)=\eta(T\pi_Q(v_n),...,T\pi_Q(v_n))$. 
...
\begin{frame}{Singular Reduction: Roadmap}
	...
	\begin{block}{Data:}
			\begin{itemize}
				...
				\item $S\subset M$ closed $N$ (possibly singular),
...
\begin{frame}[fragile]{Higher Observables Constraint Algebra}
	\begin{defpropblock}[$L_\infty$-observables constraint algebra]
		The following $L_\infty$ algebras form a constraint triple
		\begin{align*}
			\Ham_T =&~ L_\infty(M,\omega) \\
			\Ham_N =&~
			{\small
			\left\lbrace
			\begin{array}{l}
	...
	\vfill
	\pause
	Hence, The following subquotient $\frac{\Ham_N}{\Ham_0}$ of $\Ham_T$ forms an $L_\infty$-algebra.
\end{frame}
\note[itemize]{
	\item 	\begin{defblock}[Reducible v.fields ]
			\begin{displaymath}
				\X(M)_{[N]} :=
				\left\lbrace
					v \in \X(M)
...
%TODO aknowledgiare Leonid (sue slides) e Marvin (sua Tesi su Arxiv
%- . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - . - .%
...
\begin{frame}[fragile]{Geometric Multisymplectic reduction}
...
\begin{frame}{Relating Geometric and Algebraic reduction}
